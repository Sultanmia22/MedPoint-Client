...
const MobileDrawer = ({ open, onClose, role = null, onLogout }: MobileDrawerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Drawer */}
      <div
        className={`absolute left-0 top-0 h-full w-72 bg-bg shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >

        {/* Header */}
        <div className="relative border-b border-border px-4 py-4 min-h-15">
          <button
            type="button"
            onClick={onClose}
            aria-label="Close menu"
            className="absolute top-3 right-3 z-10 flex items-center justify-center w-9 h-9 rounded-lg border border-border text-text-body hover:bg-primary/10 hover:text-primary transition-colors"
          >
            <X className="h-6 w-6" />
          </button>
          ...
          {isLoggedIn && (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-heading text-xl font-semibold">MD Sultan Mia</p>
                <p className="text-sm text-text-muted">sultanmia5322@gmai.com</p>
              </div>
            </div>
          )}
        </div>
          ...
          {isLoggedIn ? (
            ...
            <>
              <div className="flex items-center gap-3 rounded-lg bg-primary/5 px-3 py-2.5">
                ...
                <span className="text-sm font-medium text-text-heading">Guest User</span>
              </div>
